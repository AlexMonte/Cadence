...
fn side_from_to_node(to_node: &GridPos, from: &GridPos) -> Option<TileSide> {
    if from.col == to_node.col + 1 && from.row == to_node.row {
        return Some(TileSide::East);
    }
    if from.col == to_node.col - 1 && from.row == to_node.row {
        return Some(TileSide::West);
    }
    if from.col == to_node.col && from.row == to_node.row - 1 {
        return Some(TileSide::North);
    }
    if from.col == to_node.col && from.row == to_node.row + 1 {
        return Some(TileSide::South);
    }
    None
}
...
pub fn pick_target_param_for_edge(
    ...
    let Some(target_side) = side_from_to_node(to_node, from) else {
        return EdgeTargetParamProbe::reject(
    ...
    let source_output_type = match source_output_type_for_target_side(&base, target_side) {
...
pub fn validate_edge_connect(
    ...
    let source_output_type = source_output_type_for_target_side(&base, target_side)?;
    if graph
        .edges
        .values()
