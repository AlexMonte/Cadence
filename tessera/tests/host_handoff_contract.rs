...
#[test]
fn preview_alternate_respects_cycle_index() {
    use tessera::prelude::*;
...
#[test]
fn preview_choice_expr_respects_cycle_index() {
    use tessera::prelude::*;
...
#[test]
fn sequence_elongate_weights_slot_durations() {
    use tessera::prelude::*;
    ...
    assert_eq!(events[0].span.duration.0, Rational::new(2, 3));
    assert_eq!(events[1].span.duration.0, Rational::new(1, 3));
}

#[test]
fn invalid_note_label_emits_container_stack_diagnostic() {
    use tessera::prelude::*;
    ...
    let diagnostics = TesseraCompiler::new()
        ...
        .expect_err("invalid note should fail normalization")
        .into_iter()
        .collect::<Vec<_>>();
    assert!(diagnostics.iter().any(|diagnostic| {
        diagnostic.kind == DiagnosticKind::InvalidNote
            && matches!(
                diagnostic.location,
                Some(DiagnosticLocation::ContainerStack { .. })
            )
...
fn atom_degrade_filters_events_transform_degrade_annotates_field() {
    ...
    root_nodes.insert(
        NodeId::new("atom"),
        RootSurfaceNodeKind::Container {
            container: ContainerId::new("atom"),
        },
    );
    root_nodes.insert(
        NodeId::new("out"),
    ...
    let atom_ir = TesseraCompiler::new()
        .compile_ir(&TesseraProgram {
            ...
            relations: vec![RootRelation::FlowsTo {
                from: StreamSource::node(NodeId::new("atom")),
                to: StreamTarget::OutputInput {
                    node: NodeId::new("out"),
                    endpoint: output_input("main"),
                },
        ...
        .expect("atom degrade should compile");
    let atom_events = atom_ir.outputs[0].events().len();
        ...
        .compile_ir(&TesseraProgram {
            ...
            relations: vec![
                RootRelation::FlowsTo {
                    from: StreamSource::node(NodeId::new("main")),
                    to: StreamTarget::TransformInput {
                        node: NodeId::new("degrade"),
                        endpoint: transform_input("main"),
                    },
                },
                RootRelation::FlowsTo {
                    from: StreamSource::node(NodeId::new("degrade")),
                    to: StreamTarget::OutputInput {
                        node: NodeId::new("out"),
                        endpoint: output_input("main"),
                    },
