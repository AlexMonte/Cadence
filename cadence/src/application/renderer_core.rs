...
use crate::{
    ...
    domain::{control::ControlModelError, score::Score, span::TransportSpan, voice::Voice},
};
...
mod tests {
    ...
    #[test]
    fn evaluated_events_carry_control_map_from_with_controls() {
        ...
        let output = renderer.evaluate_window(&span((0, 1), (1, 1))).unwrap();

        assert!(matches!(
            output[0].projected().controls().get(&ControlKey::Gain),
    ...
    #[test]
    fn degrade_affects_evaluated_events_deterministically() {
        ...
        let first = renderer.evaluate_window(&span((0, 1), (4, 1))).unwrap();
        let second = renderer.evaluate_window(&span((0, 1), (4, 1))).unwrap();
