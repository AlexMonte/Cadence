use cadence::commands::{SharedAppState, take_main_window_close_allowance};
use cadence::desktop_invoke_handler;
use serde::Serialize;
...
const ACTION_WINDOW_CLOSE_REQUESTED: &str = "window.close_requested";
...
fn app_menu<R: Runtime>(app: &AppHandle<R>) -> tauri::Result<Menu<R>> {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        Menu::with_items(app, &[&app_submenu, &file_menu, &edit_menu, &view_menu])
    }
...
fn emit_ui_action<R: Runtime>(app: &AppHandle<R>, action: &str) {
...
fn toggle_dev_inspector_window<R: Runtime>(app: &AppHandle<R>) {
    if !cfg!(debug_assertions) {
        emit_ui_action(app, "view.toggle_dev_inspector_blocked");
        return;
    }
...
fn handle_menu_event<R: Runtime>(app: &AppHandle<R>, menu_id: &str) {
    match menu_id {
        MENU_FILE_NEW => emit_ui_action(app, "file.new"),
        MENU_FILE_OPEN => emit_ui_action(app, "file.open"),
        MENU_FILE_SAVE => emit_ui_action(app, "file.save"),
        MENU_FILE_SAVE_AS => emit_ui_action(app, "file.save_as"),
        MENU_FILE_EXPORT => emit_ui_action(app, "file.export_song"),
        MENU_FILE_QUIT => emit_ui_action(app, "file.quit"),
        MENU_EDIT_UNDO => emit_ui_action(app, "edit.undo"),
        MENU_EDIT_REDO => emit_ui_action(app, "edit.redo"),
        MENU_VIEW_TOGGLE_CONSOLE => emit_ui_action(app, "view.toggle_mini_console"),
        MENU_VIEW_TOGGLE_DEV_INSPECTOR => {
            toggle_dev_inspector_window(app);
            emit_ui_action(app, "view.toggle_dev_inspector");
        }
        _ => {}
    }
}
    ...
    let app = tauri::Builder::default()
        ...
        .on_window_event(|window, event| {
            ...
            if let tauri::WindowEvent::CloseRequested { api, .. } = event {
                ...
                if take_main_window_close_allowance(&state) {
                    return;
                }
                api.prevent_close();
                emit_ui_action(window.app_handle(), ACTION_WINDOW_CLOSE_REQUESTED);
            }
        })
