...
use crate::adapter::tessera::graph_defaults::normalize_graph_piece_sides;
use crate::application::authoring::ops::engine_for_target;
use crate::application::error::{AppError, AppResult};
use crate::application::{history, state::AppStore};
...
fn undo_graph_op_internal(store: &mut AppStore) -> AppResult<HistoryStatusDto> {
    ...
    let mut candidate = project
        ...
        .clone();
    let engine = engine_for_target(project, &record.target);
    normalize_graph_piece_sides(&mut candidate, engine.registry());
    if let Err(errors) = engine.apply_ops(&mut candidate, record.record.undo_ops.as_slice()) {
    ...
    if let Some(project) = store.current_project.as_mut()
        && let Some(graph) = project.graph_mut(&record.target)
    {
        *graph = candidate;
    }
    store.clear_compile_cache_for_target(&record.target);
...
fn redo_graph_op_internal(store: &mut AppStore) -> AppResult<HistoryStatusDto> {
    ...
    let engine = engine_for_target(project, &record.target);
    normalize_graph_piece_sides(&mut candidate, engine.registry());
    if let Err(errors) = engine.apply_ops(&mut candidate, record.record.do_ops.as_slice()) {
        store.graph_history_future.push(record);
        return Err(AppError::InvalidInput(format!(
            "graph redo failed: {}",
            errors
...
pub fn history_status(state: &SharedAppState) -> Result<HistoryStatusDto, String> {
    let store = state
        .store
        .lock()
...
pub fn history_undo(state: &SharedAppState) -> Result<HistoryStatusDto, String> {
    let mut store = state
        .store
        .lock()
        .map_err(|_| "app state lock poisoned".to_string())?;
...
pub fn history_redo(state: &SharedAppState) -> Result<HistoryStatusDto, String> {
    let mut store = state
        .store
        .lock()
        .map_err(|_| "app state lock poisoned".to_string())?;
...
mod tests {
    ...
    use crate::application::history::record_graph_mutation;
    use crate::domain::project::{
        CadenceGraphTarget, CadenceProjectDocument, TargetedGraphOpRecord,
    };
    use tessera::graph::{Edge, Graph, GraphOp, GraphOpRecord, Node};
        ...
        nodes.insert(
            ...
            Node {
                ...
                input_sides: BTreeMap::from([
                    ("pattern".to_string(), tessera::types::TileSide::LEFT),
                    ("factor".to_string(), tessera::types::TileSide::BOTTOM),
                ]),
                output_side: None,
                label: None,
                node_state: None,
    ...
    fn graph_history_undo_redo_replays_inverse_ops() {
                ...
                &[
                    GraphOp::NodePlace {
                        ...
                        inline_params: BTreeMap::from([
                            ...
                            ("widget".to_string(), Value::String("dial".to_string())),
                        ]),
                        pattern_source: None,
                    },
                    GraphOp::EdgeConnect {
        ...
        *store
        ...
        let graph_before_undo = store
            .current_project
            .as_ref()
            .expect("project")
            .runtime_graph()
            .clone();
        let _ = engine.analyze_cached(&graph_before_undo, &mut store.runtime_compile_cache);
        ...
        let graph_before_redo = store
            .current_project
            .as_ref()
            .expect("project")
            .runtime_graph()
            .clone();
        let _ = engine.analyze_cached(&graph_before_redo, &mut store.runtime_compile_cache);
        assert!(!store.runtime_compile_cache.is_empty());

        let redo_status = history_redo_internal(&mut store).expect("redo");
        assert!(redo_status.can_undo);
        assert!(store.runtime_compile_cache.is_empty());
        let redone_graph = store
            .current_project
            .as_ref()
            .expect("project")
            .runtime_graph();
